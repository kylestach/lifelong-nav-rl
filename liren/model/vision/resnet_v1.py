...
from liren.model.vision.film_conditioning_layer import FilmConditioning
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(self, observations: jnp.ndarray, train: bool = True, cond_var=None):
        ...
        if self.pooling_method == "spatial_learned_embeddings":
            ...
            if len(x.shape) == 4:
                x = jnp.reshape(x, [x.shape[0], -1])  # flatten w/o batch dim
            elif len(x.shape) == 3:
                x = jnp.reshape(x, (-1))  # flatten
            else:
                raise RuntimeError("image of invalid shape")
        else:
